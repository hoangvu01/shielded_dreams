...
parser.add_argument(
    "--action-repeat", type=int, default=1, metavar="R", help="Action repeat"
)
parser.add_argument(
...
encoder = Encoder(
    env.observation_size, args.embedding_size, args.cnn_activation_function
).to(device=args.device)
...
param_list = (
    ...
    + list(violation_model.parameters())
...
if args.test:
    ...
    shield = BoundedPrescienceShield(
        ...
        paths_to_sample=args.paths_to_sample,
    )
    with torch.no_grad():
        rewards = []
        violations = []
...
for episode in tqdm(
    ...
    for s in tqdm(range(args.collect_interval)):
        ...
        if args.worldmodel_LogProbLoss:
            ...
            observation_loss = (
                -observation_dist.log_prob(observations[1:]).sum(dim=2).mean(dim=(0, 1))
            )
        else:
            observation_loss = (
                F.mse_loss(
    ...
    lineplot(
        metrics["episodes"][-len(metrics["violation_loss"]) :],
        metrics["violation_loss"],
        "violation_loss",
        results_dir,
    )
